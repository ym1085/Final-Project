	...
	<section class="mysec" style="margin-top: 5%;">
		<div class="mysecDiv">
			...
			<form name="frm1" method="post"
				...
				<input type="text" id="NUemail" name="customerId" placeholder="이메일 입력해주세요" value="${dateSearchVO.customerId }">
				<input type="submit" value="조회" id="listsubmit">
				<br><span style="color: red;font-weight: bold;">*관람하시는날이 아닌 예매 하신 날로 검색이 가능합니다.</span>
			</form>
			...
			<table class="mytable">
				...
				<!-- 반복 시작-->
					<tr>
						...
						<c:if test="${today<selectDate && map['PAY_CONDITION']=='Y'}">
						<!-- 예매 하면 바로 주문취소 넣어주고 만약 공연을봤다 면 후기작성으로-->
						<a class="mysecCansle" href="<c:url value='/member/myreserCancle.do?reservationSeq=${map["RESERVATION_SEQ"] }&noneuserid=${map["NONEUSERID"] }' />">예매취소</a>
						</c:if>
						<c:if test="${today==selectDate && map['PAY_CONDITION']=='Y'}">
						<!-- 예매 하면 바로 주문취소 넣어주고 만약 공연을봤다 면 후기작성으로-->
						<span>취소불가</span>
